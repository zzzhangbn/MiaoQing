    ...
    <div style="margin-left: 1%;">
                    ...
                    <el-form :inline="true"  label-width="180px">
                        <el-form-item label="使用时间" >
                            <el-date-picker
                                    align="right"
                                    type="date"
                                    v-model="tab020605DataList[item.date]"
                                    :picker-options="pickerOptions"
                                    value-format=“yyyy-MM-dd”
                                    :disabled="totalData.length-index-1 < tab020605CurrentStep "
                                    placeholder="选择日期">
                            </el-date-picker>
                        </el-form-item>
                        <el-form-item label="化调剂名称" >
                            <el-autocomplete
                                    style="width: 220px;"
                                    class="inline-input"
                                    :fetch-suggestions="querySearchName"
                                    v-model="tab020605DataList[item.name]"
                                    :disabled="totalData.length-index-1 < tab020605CurrentStep "
                                    placeholder="请输入内容"
                            ></el-autocomplete>
                        </el-form-item>
                        <el-form-item label="亩用量/kg" >
                            <el-input
                                ...
                                replace(/\./g, '').replace('$#$', '.').replace(/^(\-)*(\d+)\.(\d\d).*$/, '$1$2.$3').replace(/^\./g, '')"
                                    v-model="tab020605DataList[item.quantity]"
                                    :disabled="totalData.length-index-1 < tab020605CurrentStep ">
                            </el-input>
                        </el-form-item>
                        <el-form-item label="使用方法" >
                            <el-autocomplete
                                    ...
                                    v-model="tab020605DataList[item.way]"
                                    :disabled="totalData.length-index-1 < tab020605CurrentStep "
                                    placeholder="请输入内容"
                            ></el-autocomplete>
                        </el-form-item>
                        <el-form-item label="生育进程" >
                            <el-autocomplete
                                    ...
                                    v-model="tab020605DataList[item.process]"
                                    :disabled="totalData.length-index-1 < tab020605CurrentStep "
                                    placeholder="请输入内容"
                            ></el-autocomplete>
                        </el-form-item>
        ...
        <div style="width: 100%;height: 100px;margin-top: 25px;">
            <div style="margin-top: 2.3%;float: right;margin-right: 10%;">
                <template v-if="currentStep >= tab020605CurrentStep && this.tab020605CurrentStep != 3">
    ...
    export default{
        ...
        data() {
            return {
                ...
                totalData:[],
                stepTitle: [ '化调剂1','化调剂2','化调剂3'],
                ways: [{ "value": "撒施" },{ "value": "喷雾" },{ "value": "喷施" },
                    { "value": "拌种" },{ "value": "飞防" }],
                huaNames: [{ "value": "赤霉素" },{ "value": "多效唑" },{ "value": "乙烯利" }],
        ...
        methods: {
            ...
            getFormList(e){
                this.$confirm("是否上报？").then(_=>{
                    ...
                    this.tab020605DataList.modification_time = date;
                    this.tab020605DataList.in_use = 1;

                    if (this.tab020605DataList.field_7_1 != null){
                        this.tab020605DataList.field_7_1 =this.tab020605DataList.field_7_1.toString().replace('“','').replace('”','');
                    }
                    if (this.tab020605DataList.field_8_1 != null){
                        this.tab020605DataList.field_8_1 =this.tab020605DataList.field_8_1.toString().replace('“','').replace('”','');
                    }
                    if (this.tab020605DataList.field_9_1 != null){
                        this.tab020605DataList.field_9_1 =this.tab020605DataList.field_9_1.toString().replace('“','').replace('”','');
                    }

                    var list = this.tab020605DataList;
                    var str = '';
            ...
            saveInputData(list){
                axios({
                    ...
                    this.gettab020605CurrentStep(this.people,this.point)
            ...
            gettab020605CurrentStep(people,point){
                axios({
                    url:'/getAllDataTwo0605',
                    method:'get',
                    params:{
                        people:people,
                        point:point,
                    ...
                    res=>{
						if(res.data.length > 0){
							this.tab020605DataList = [];
							this.totalData =[];
							this.tab020605DataList = res.data[0];
							//console.log(this.totalData);
							this.tab020605CurrentStep=0;
							for(var j = 0 ; j < 3 ; ++j){
							    var start = 7+j;
							    var str = "field_"+start+"_1";
							    var str1 = "field_"+start+"_2";
							    if(this.tab020605DataList[str] !=null && this.tab020605DataList[str] !=''){
							        this.tab020605CurrentStep += 1;
							        this.latestData = this.tab020605DataList[str];
							        var currenrData = {};
							        currenrData.date = "field_"+start+"_1";
							        currenrData.name = "field_"+start+"_2";
							...
							//console.log(this.totalData);
							if(this.tab020605CurrentStep > this.currentStep){
							    this.currentStep = this.tab020605CurrentStep;
							}
							
							//console.log(this.tab020605CurrentStep);
							
							this.maxStep = this.currentStep;
							for(var i = 1 ; i <= this.tab020605CurrentStep ; ++i){
							    var item = ""+i+"";
