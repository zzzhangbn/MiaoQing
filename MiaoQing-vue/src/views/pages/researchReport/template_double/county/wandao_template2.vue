    ...
    <el-card style="margin:20px 0 0 0;cursor:pointer;">
      <div class="content" ref="content">
        ...
        <div>
          ...
          <input v-model="inputValues[0]" placeholder="手动输入苗情评价" type="text" class="input" style="width:1000px;">。
          </p>
        </div>
        <div>
          <h2>二、苗情分析</h2>
...
export default {
  ...
  methods:{
    getData(){
      ...
      axios({
        url:'/template_double',
        params:{templateName:'双季晚稻分蘖－孕穗期苗情',account:account},
        method:'get'
      }).then(res=>{
    ...
    saveReport(){
      ...
      this.$confirm('确定上传？').then(_=>{
        ...
        axios({
          ...
          this.$router.replace({name:'/tableTwoMain',params:{activeNum: 4}});
        })
      }).catch(_=>{
        this.$message('取消上传');
      })
