    ...
    <el-card style="margin:20px 0 0 0;cursor:pointer;">
      <div class="content" ref="content">
        ...
        <div>
          ...
          <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            全县（市、区），一季稻种植面积<u>{{reportData.totalArea_1_1}}</u>万亩，
            比去年 <u>{{reportData.totalAreaSub_1_1}}</u>万亩 。
          ...
          <h3>2. 种植类型与面积比较:</h3>
          <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            据统计，一季稻人工移栽约<u>{{reportData.cateList_1_1[0]}}</u>万亩，占 <u>{{reportData.catePercent_1_1[0]}}</u>%,比去年<u>{{reportData.cateList2_1_1[0]}}</u>个百分点；
          ...
          <h3>4．播栽期、栽插进度:</h3> 
          <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            截至 <u>{{month}}</u>月<u>{{day}}</u>日，全县（市、区）一季稻播栽约<u>{{reportData.table_3_1_area}}</u>万亩，占总面积的 <u>{{reportData.areaPercent_3_1}}</u> %，
...
export default {
  ...
  methods:{
    getData(){
      ...
      axios({
        url:'/template1',
        params:{templateName:'一季稻栽播期期苗情',country:area},  
        method:'get'
      }).then(res=>{
