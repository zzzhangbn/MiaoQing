    ...
    <el-card style="margin:20px 0 0 0;cursor:pointer;">
      <div class="content" ref="content">
        <h1 style="width: 80%;margin: 0 auto;text-align: center;">
          <input ref="title" class="title" type="text" style="width:70%;font-size:25px;text-align: center;border:none;font-weight:bold" :value="thisYear+title">
        </h1>
        ...
        <div>
          ...
          <p style="font-size: 20px;line-height: 30px;">
            &nbsp;&nbsp;全县（市、区）一季稻面积<input v-model="inputValues[0]" type="text" ref="input1" style="width:40px;background:none;border: none;border-bottom: 1px solid #000;font-size: 20px;outline: none;">万亩，
            较去年<input type="text" v-model="inputValues[1]" ref="input2" style="width:100px;border: none;background:none;border-bottom: 1px solid #000;font-size: 20px;outline: none;">万亩。
          ...
          <p style="font-size: 20px;line-height: 30px;">
            ...
            &nbsp;&nbsp;据调查，平均亩有效穗<u>{{reportData.data_3.yxh}}</u> 万，每穗总粒数 <u>{{reportData.data_3.zls}}</u>粒，结实率 <u>{{reportData.data_3.jsl}}</u>%，
          ...
          <p style="font-size: 20px;line-height: 30px;">
            &nbsp;&nbsp;其中，8月上旬抽穗扬花的中籼稻，全县（市、区）约 <u>{{reportData.data_4.area}}</u> 万亩左右，占 <u>{{reportData.data_4.percent}}</u>%，
            <input type="text" style="width:98%;" ref="input4" placeholder="请输入分布地区、前茬类型及当前天气对抽穗扬花的评价" v-model="inputValues[3]">

            &nbsp;&nbsp;据初步调查，
            <!-- 平均亩有效穗<u>{{reportData.data_4.yxh}}</u> 万，每穗总粒数 <u>{{reportData.data_4.qls}}</u>粒， -->
            结实率 <u>{{reportData.data_4.jsl}}</u>%，
            <!-- 千粒重按 <u>{{reportData.data_4.qlz}}</u> 克计，理论亩产 <u>{{reportData.data_4.chanliang_1}}</u>公斤，比去年同期（<u>{{reportData.data_4.chanliang_2}}</u>公斤）<u>{{reportData.data_4.subPercent}}</u> % -->
          </p>
          <p style="font-size: 20px;line-height: 30px;">
            &nbsp;&nbsp;8月中下旬抽穗扬花的中籼稻，全县（市、区）约 <u>{{reportData.data_5.area}}</u> 万亩，占 <u>{{reportData.data_5.percent}} </u>%。
            <input type="text" placeholder="手动输入分布地区、前茬类型及当前天气对抽穗扬花的评价" ref="input5" v-model="inputValues[4]">
          </p>

          <p style="font-size: 20px;line-height: 30px;">
            &nbsp;&nbsp;9月上中旬抽穗扬花的中籼稻，全县（市、区） <u>{{reportData.data_6.area}}</u> 万亩，约占 <u>{{reportData.data_6.percent}} </u>%。
            <input type="text" placeholder="手动输入分布地区、前茬类型及当前天气对抽穗扬花的评价" ref="input6" v-model="inputValues[5]">
          </p>

          <h3>3．叶龄、群体茎蘖数</h3>
          <p>
              <span style="font-weight:bold;">籼稻:</span> 移栽的平均亩茎蘖数<u>{{reportData.data_7[0]}}</u>万，与去年同期<u v-if="reportData.data_7[1]>=0">{{"多"+reportData.data_7[1]}}</u> <u v-else>{{"少"+(0-reportData.data_7[1])}}</u>万；
              叶龄<u>{{reportData.data_7[2]}}</u>叶，较去年同期 <u v-if="reportData.data_7[3]>=0">{{"多"+reportData.data_7[3]}}</u> <u v-else>{{"少"+(0-reportData.data_7[3])}}</u>  叶。
              其中，人工移栽的平均亩茎蘖数为 <u>{{reportData.data_7[4]}}</u>万，叶龄<u>{{reportData.data_7[5]}}</u> 叶；
              机插的平均亩茎蘖数 <u>{{reportData.data_7[6]}}</u> 万，叶龄 <u>{{reportData.data_7[7]}}</u> 叶；
              抛栽的平均亩茎蘖数 <u>{{reportData.data_7[8]}}</u> 万，叶龄 <u>{{reportData.data_7[9]}}</u> 叶。
              直播的平均亩茎蘖数<u>{{reportData.data_9[0]}}</u>万，较去年<u v-if="reportData.data_9[1]>=0">{{"多"+reportData.data_9[1]}}</u> <u v-else>{{"少"+(0-reportData.data_9[1])}}</u>万；
              叶龄<u>{{reportData.data_9[2]}}</u>叶，较去年<u v-if="reportData.data_9[3]>=0">{{"多"+reportData.data_9[3]}}</u> <u v-else>{{"少"+(0-reportData.data_9[3])}}</u>叶。
          </p>
          <p>
            <span style="font-weight:bold;">粳稻:</span>以常规粳稻为主，移栽的平均亩茎蘖数<u>{{reportData.data_8[0]}}</u>万，与去年同期<u v-if="reportData.data_8[1]>=0">{{"多"+reportData.data_8[1]}}</u> <u v-else>{{"少"+(0-reportData.data_8[1])}}</u>万；
              叶龄<u>{{reportData.data_8[2]}}</u>叶，较去年同期 <u v-if="reportData.data_8[3]>=0">{{"多"+reportData.data_8[3]}}</u> <u v-else>{{"少"+(0-reportData.data_8[3])}}</u>  叶。
              其中，人工移栽的平均亩茎蘖数为 <u>{{reportData.data_8[4]}}</u>万，叶龄<u>{{reportData.data_8[5]}}</u> 叶；
              机插的平均亩茎蘖数 <u>{{reportData.data_8[6]}}</u> 万，叶龄 <u>{{reportData.data_8[7]}}</u> 叶；
              抛栽的平均亩茎蘖数 <u>{{reportData.data_8[8]}}</u> 万，叶龄 <u>{{reportData.data_8[9]}}</u> 叶。
              直播的平均亩茎蘖数<u>{{reportData.data_9[4]}}</u>万，较去年<u v-if="reportData.data_9[5]>=0">{{"多"+reportData.data_9[5]}}</u> <u v-else>{{"少"+(0-reportData.data_9[5])}}</u>万；
              叶龄<u>{{reportData.data_9[6]}}</u>叶，较去年<u v-if="reportData.data_9[7]>=0">{{"多"+reportData.data_9[7]}}</u> <u v-else>{{"少"+(0-reportData.data_9[7])}}</u>叶。
          </p>
        </div>
        <div>
          <h2>二、当前预估产分析</h2>
  ...
  methods:{
    getData(){
      ...
      axios({
        url:'/template1',
        method:'get',
        params:{templateName:'一季稻抽穗期苗情-1',
        country:area}
      }).then(res=>{
    ...
    saveReport(){
      ...
      this.$confirm('确定上传？').then(_=>{
        this.$refs.title.setAttribute('value',this.thisYear+'年度一季稻抽穗期苗情（一）');
