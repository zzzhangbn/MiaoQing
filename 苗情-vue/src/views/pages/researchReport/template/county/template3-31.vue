    ...
    <el-card style="margin:20px 0 0 0;cursor:pointer;">
      <div class="content" ref="content">
        <h1 style="width: 80%;margin: 0 auto;text-align: center;">
          <input ref="title" class="title" type="text" style="width:70%;background:none;font-size:25px;text-align: center;border:none;font-weight:bold" :value="thisYear+title" >
        </h1>
        ...
        <div>
          ...
          <p style="font-size: 20px;line-height: 30px;">
            ...
            与去年相比<input type="text" v-model="inputValues[1]" ref="input2" style="width:100px;background:none;border: none;border-bottom: 1px solid #000;font-size: 20px;outline: none;">万亩。
          ...
          <p style="font-size: 20px;line-height: 30px;">
            &nbsp;&nbsp;其中，8月下旬抽穗扬花的中籼稻，约 <u>{{reportData.data_5.area}}</u> 万亩左右，占 <u>{{reportData.data_5.percent}}</u>%，
            <input type="text" style="width:98%;" ref="input5" placeholder="请输入分布地区、前茬类型及当前天气对抽穗扬花的评价" v-model="inputValues[4]">
            &nbsp;&nbsp;据调查，平均亩有效穗<u>{{reportData.data_5.yxh}}</u> 万，每穗总粒数 <u>{{reportData.data_5.qls}}</u>粒，结实率 <u>{{reportData.data_5.jsl}}</u>%，
            千粒重按 <u>{{reportData.data_5.qlz}}</u> 克计，理论亩产 <u>{{reportData.data_5.chanliang_1}}</u>公斤，比去年同期（<u>{{reportData.data_5.chanliang_2}}</u>公斤）<u>{{reportData.data_5.subPercent}}</u> %
          </p>
          <p style="font-size: 20px;line-height: 30px;">
            &nbsp;&nbsp;9月上中旬抽穗扬花的中籼稻，全县（市、区）约 <u>{{reportData.data_6.area}}</u> 万亩，占 <u>{{reportData.data_6.percent}} </u>%。
  ...
  methods:{
    getData(){
      ...
      axios({
        url:'/template1',
        method:'get',
        params:{templateName:'一季稻抽穗期苗情-3',country:area}
      }).then(res=>{
    ...
    saveReport(){
      ...
      this.$confirm('确定上传？').then(_=>{
        this.$refs.title.setAttribute('value',this.thisYear+'年度一季稻抽穗期苗情（三）');
