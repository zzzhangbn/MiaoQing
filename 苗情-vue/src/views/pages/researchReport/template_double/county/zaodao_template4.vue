    ...
    <el-card style="margin:20px 0 0 0;cursor:pointer;">
      <div class="content" ref="content">
        ...
        <div>
          <h2>一、当前苗情</h2>
          <h3>1.生育进程评价</h3>
          <input v-model="inputValues[0]" placeholder="手动输入生育进程及评价" type="text" class="input" style="width:1000px">。
          <h3>2．群体描述:</h3>
          <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
             当前平均亩茎蘖数 <u>{{reportData.datas_2[0]}}</u>万, 比上年同期<u>{{reportData.sub_2[0]}}</u> 万；
             平均叶龄   <u>{{reportData.datas_2[1]}}</u>   叶，比上年同期<u>{{reportData.sub_2[1]}}</u>     叶。
             其中，人工移栽的平均亩茎蘖数   <u>{{reportData.datas_2[2]}}</u>   万，叶龄  <u>{{reportData.datas_2[3]}}</u>    叶；
             盘育抛栽的平均亩茎蘖数   <u>{{reportData.datas_2[4]}}</u>   万，叶龄  <u>{{reportData.datas_2[5]}}</u>    叶；
             机械插秧的平均亩茎蘖数为   <u>{{reportData.datas_2[6]}}</u>   万，叶龄   <u>{{reportData.datas_2[7]}}</u>   叶；
             撒直播的平均亩茎蘖数为  <u>{{reportData.datas_2[8]}}</u>    万，叶龄   <u>{{reportData.datas_2[9]}}</u>   叶，       
            <input v-model="inputValues[1]" placeholder="手动输入生育进程及评价" type="text" class="input" style="width:1000px;">。

          </p>
          <h3>3.面上苗情介绍</h3>
          <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
              全县（市、区）调查，当前一类苗 <u>{{reportData.datas_3[0]}}</u>     万亩，占   <u>{{reportData.percents[0]}}</u>   %；
...
export default {
  ...
  methods:{
    getData(){
      ...
      axios({
        url:'/template_double',
        params:{templateName:'早稻孕穗－抽穗期苗情',account:account},
        method:'get'
      }).then(res=>{
    ...
    saveReport(){
      ...
      this.$confirm('确定上传？').then(_=>{
        this.$refs.title.setAttribute('value',this.thisYear+'年度双季早稻孕穗－抽穗期苗情');
