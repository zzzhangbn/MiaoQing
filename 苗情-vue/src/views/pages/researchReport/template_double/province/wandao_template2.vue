    ...
    <el-card style="margin:20px 0 0 0;cursor:pointer;">
      <div class="content" ref="content">
        ...
        <div>
          ...
          <h3>1．叶龄、群体茎蘖数介绍:</h3>
          <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
              移抛栽田平均叶龄 <u>{{reportData.datas_1[0]}}</u>    叶，亩茎蘖数平均   <u>{{reportData.datas_1[1]}}</u>  万。
              其中，人工移栽的叶龄  <u>{{reportData.datas_1[2]}}</u>   叶，亩茎蘖数  <u>{{reportData.datas_1[3]}}</u>   万；
              盘育抛栽的叶龄  <u>{{reportData.datas_1[4]}}</u>   叶，亩茎蘖数 <u>{{reportData.datas_1[5]}}</u>    万；
              机械插秧的叶龄   <u>{{reportData.datas_1[6]}}</u>  叶，亩茎蘖数 <u>{{reportData.datas_1[7]}}</u>    万。
              直播田平均叶龄   <u>{{reportData.datas_1[8]}}</u>  叶，亩茎蘖数  <u>{{reportData.datas_1[9]}}</u>   万。
          </p>
          <h3>2．苗情分析:</h3>
          <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            据农情调度统计，全省双季晚稻播栽面积约
          <input v-model="inputValues[0]" placeholder="手动输入苗情评价" type="text" class="input" style="width:200px;">,
          比去年
          <input v-model="inputValues[1]" placeholder="手动输入苗情评价" type="text" class="input" style="width:200px;">
          万亩
            全县（市、区）共  <u>{{reportData.datas_2[0]}}</u>   万亩，
            一类苗   <u>{{reportData.datas_2[1]}}</u>  万亩，占   <u>{{reportData.datas_2[2]}}</u>  %；
            二类苗  <u>{{reportData.datas_2[3]}}</u>   万亩，占  <u>{{reportData.datas_2[4]}}</u>   %；
            三类苗   <u>{{reportData.datas_2[5]}}</u>  万亩，占  <u>{{reportData.datas_2[6]}}</u>   %。
            一、二类苗面积万亩  <u>{{reportData.datas_2[7]}}</u>   万亩，占比 <u>{{reportData.datas_2[8]}}</u>    %，
            比去年同期<u>{{reportData.sub_2[1]}}</u>个百分点，总体苗情
          <input v-model="inputValues[2]" placeholder="手动输入苗情评价" type="text" class="input" style="width:1000px;">。
          </p>
        </div>
        <div>
          <h2>二、苗情分析</h2>
...
export default {
  ...
  methods:{
    getData(){
      ...
      axios({
        url:'/template_double',
        params:{templateName:'双季晚稻分蘖－孕穗期苗情',account:''},
        method:'get'
      }).then(res=>{
    ...
    saveReport(){
      ...
      this.$confirm('确定上传？').then(_=>{
        this.$refs.title.setAttribute('value',this.thisYear+'年度双季晚稻分蘖－孕穗期苗情');
        ...
        axios({
          ...
          this.$router.replace({name:'/researchReport',params:{activeNum: 4}});
        })
      }).catch(_=>{
        this.$message('取消上传');
      })
