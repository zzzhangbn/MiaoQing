    ...
    <div style="margin-left: 1%;">
                    ...
                    <el-form :inline="true"  label-width="180px">
                        <el-form-item label="时间" >
                            <el-date-picker
                                    align="right"
                                    type="date"
                                    v-model="tab020604DataList[item.date]"
                                    :picker-options="pickerOptions"
                                    value-format=“yyyy-MM-dd”
                                    :disabled="totalData.length-index-1 < tab020604CurrentStep "
                                    placeholder="选择日期">
                            </el-date-picker>
                        </el-form-item>
                        <el-form-item label="N" >
                            <el-input style="width: 220px;"
                                      ...
                                      v-model="tab020604DataList[item.N]"
                                      :disabled="totalData.length-index-1 < tab020604CurrentStep">
                            </el-input>
                        </el-form-item>
                        <el-form-item label="P" >
                            <el-input style="width: 220px;"
                                      ...
                                      oninput="value=value.replace(/[^\d.]/g, '').replace(/\.{2,}/g, '.').replace('.', '$#$').
                                      replace(/\./g, '').replace('$#$', '.').replace(/^(\-)*(\d+)\.(\d\d).*$/, '$1$2.$3').replace(/^\./g, '')"
                                      v-model="tab020604DataList[item.P]"
                                      :disabled="totalData.length-index-1 < tab020604CurrentStep">
                            </el-input>
                        </el-form-item>
                        <el-form-item label="K" >
                            <el-input style="width: 220px;"
                                      ...
                                      oninput="value=value.replace(/[^\d.]/g, '').replace(/\.{2,}/g, '.').replace('.', '$#$').
                                      replace(/\./g, '').replace('$#$', '.').replace(/^(\-)*(\d+)\.(\d\d).*$/, '$1$2.$3').replace(/^\./g, '')"
                                      v-model="tab020604DataList[item.K]"
                                      :disabled="totalData.length-index-1 < tab020604CurrentStep">
                            </el-input>
                        </el-form-item>
                        <el-form-item label="方法" >
                            <el-autocomplete
                                    style="width: 220px;"
                                    class="inline-input"
                                    v-model="tab020604DataList[item.way]"
                                    :disabled="totalData.length-index-1 < tab020604CurrentStep"
                        ...
                        <el-form-item label="生育进程" >
                            <el-autocomplete
                                    ...
                                    v-model="tab020604DataList[item.process]"
                                    :disabled="totalData.length-index-1 < tab020604CurrentStep">
                            </el-autocomplete>
                        </el-form-item>
                    </el-form>
        ...
        <div style="width: 100%;height: 100px;margin-top: 25px;">
            <div style="margin-top: 2.3%;float: right;margin-right: 10%;">
                <template v-if="currentStep >= tab020604CurrentStep && this.tab020604CurrentStep != 5">
        ...
        methods: {
            ...
            getFormList(e){
                this.$confirm("是否上报？").then(_=>{
                    ...
                    this.tab020604DataList.modification_time = date;
                    this.tab020604DataList.in_use = 1;

                    if (this.tab020604DataList.field_7_1 != null){
                        this.tab020604DataList.field_7_1 =this.tab020604DataList.field_7_1.toString().replace('“','').replace('”','');
                    }
                    if (this.tab020604DataList.field_8_1 != null){
                        this.tab020604DataList.field_8_1 =this.tab020604DataList.field_8_1.toString().replace('“','').replace('”','');
                    }
                    if (this.tab020604DataList.field_9_1 != null){
                        this.tab020604DataList.field_9_1 =this.tab020604DataList.field_9_1.toString().replace('“','').replace('”','');
                    }
                    if (this.tab020604DataList.field_10_1 != null){
                        this.tab020604DataList.field_10_1 =this.tab020604DataList.field_10_1.toString().replace('“','').replace('”','');
                    }
                    if (this.tab020604DataList.field_11_1 != null){
                        this.tab020604DataList.field_11_1 =this.tab020604DataList.field_11_1.toString().replace('“','').replace('”','');
                    }
                    var list = this.tab020604DataList;
                    var str = '';
            ...
            saveInputData(list){
                axios({
                    ...
                    this.gettab020604CurrentStep(this.people,this.point)
            ...
            gettab020604CurrentStep(people,point){
                axios({
                    url:'/getAllDataTwo0604',
                    method:'get',
                    params:{
                        people:people,
                        point:point,
                    ...
                    res=>{
						if(res.data.length > 0){
							this.tab020604DataList = [];
							this.totalData =[];
							this.tab020604DataList = res.data[0];
							//console.log(this.totalData);
							this.tab020604CurrentStep=0;
							for(var j = 0 ; j < 5 ; ++j){
							    var start = 7+j;
							    var str = "field_"+start+"_1";
							    if(this.tab020604DataList[str] !=null && this.tab020604DataList[str] !=''){
							        this.tab020604CurrentStep += 1;
							        this.latestData = this.tab020604DataList[str];
							        var currenrData = {};
							...
							//console.log(this.totalData);
							if(this.tab020604CurrentStep > this.currentStep){
							    this.currentStep = this.tab020604CurrentStep;
							}
							
							//console.log(this.tab020604CurrentStep);
							
							this.maxStep = this.currentStep;
							for(var i = 1 ; i <= this.tab020604CurrentStep ; ++i){
							    var item = ""+i+"";
