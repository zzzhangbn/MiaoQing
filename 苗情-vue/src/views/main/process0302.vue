<template>
    <div style="margin-left: 1%;">
        <div style="width: 98%;height: 50px;">
            <span style="font-size: 20px;color: rgb(0, 102, 102);margin-right: 48%;">年度监测县(市、区)水稻苗情评价动态列表（移栽稻）</span>
                ...
                <el-collapse-item  :name="tableList.length-index-1" :title=stepTitle[tableList.length-index-1]>
                        <el-form :inline="true"  label-width="180px">
                            <template v-if="tableList.length-index-1 == 0">
                                ...
                                <el-input  v-model="tab2DataList.field_3" align="right"
                                :disabled="copyData.field_3 != null && copyData.field_3!= ''" placeholder="请输入数字（小数点后四位）"   oninput="value=value.replace(/[^\d.]/g, '').replace(/\.{2,}/g, '.').replace('.', '$#$').
                            ...
                            <el-form-item label="一类苗（万亩）">
                                    ...
                                    :disabled="copyData[item.field2] != null && copyData[item.field2] != ''" placeholder="请输入数字（小数点后两位）"   oninput="value=value.replace(/[^\d.]/g, '').replace(/\.{2,}/g, '.').replace('.', '$#$').
                            ...
                            <el-form-item label="二类苗（万亩）">
                                    ...
                                    :disabled="copyData[item.field4] != null && copyData[item.field4] != ''" placeholder="请输入数字（小数点后两位）"   oninput="value=value.replace(/[^\d.]/g, '').replace(/\.{2,}/g, '.').replace('.', '$#$').
                                    replace(/\./g, '').replace('$#$', '.').replace(/^(\-)*(\d+)\.(\d\d).*$/, '$1$2.$3').replace(/^\./g, '')"></el-input>
                            ...
                            <el-form-item label="三类苗（万亩）">
                                    ...
                                    :disabled="copyData[item.field6] != null && copyData[item.field6] != ''" placeholder="请输入数字（小数点后两位）"   oninput="value=value.replace(/[^\d.]/g, '').replace(/\.{2,}/g, '.').replace('.', '$#$').
                                    replace(/\./g, '').replace('$#$', '.').replace(/^(\-)*(\d+)\.(\d\d).*$/, '$1$2.$3').replace(/^\./g, '')"></el-input>
  ...
  export default{
      ...
      return {
        ...
        // 步骤标题
      stepTitle: ['进程1（移栽后7天）','进程2（移栽后14天）', '进程3（移栽后21天）', '进程4（移栽后28天）', '进程5（移栽后35天）','进程6（移栽后42天）','进程7（移栽后52天）','进程8（移栽后62天）'],
      };
    },
    methods: {
        clearInput(){//改变全县总面积，清空所有输入框
            var start = 4+this.tab2CurrentStep;
        ...
        getFormList(e){
            this.$confirm('是否上报？').then(_ => {
                ...
                && this.tab2DataList[str1]!=''&& this.tab2DataList[str3]!=null && this.tab2DataList[str3]!=''&& this.tab2DataList[str5]!=null&& this.tab2DataList[str5]!=''){
                    ...
                    if(sum != this.tab2DataList.field_3){
        ...
        getPercent(field1,field2){
            this.tab2DataList[field2] = (Math.round((this.tab2DataList[field1]/this.tab2DataList.field_3) * 100*100)/100).toFixed(2)+'%';
        },
        // 获取当前时间
        getNowTime: function () {
              let dateTime
